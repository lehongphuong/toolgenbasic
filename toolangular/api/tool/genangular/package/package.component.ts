...
export class PackageComponent implements OnInit {	
  ...
  displayedColumns: string[] = ['id', 'idcoaching', 'name', 'price'];	
   ...
   */	
  getPackage() {	
    this.api.excuteAllByWhat({ 'idCompany': this.api.idCompany }, '100')	
      .subscribe(data => {	
        // set data for table	
   ...
   */	
  onInsertData() {	
    const dialogRef = this.dialog.open(PackageDialog, {	
      width: '400px',	
      data: { type: 0, id: 0 }	
    });	
...
export class PackageDialog implements OnInit {	
  ...
  constructor(	
    public dialogRef: MatDialogRef<PackageDialog>,	
  ...
  onOkClick(): void {	
    ...
    // this.input.born = this.api.formatDate(this.input.born);	
      ...
      this.api.excuteAllByWhat(this.input, ''+ Number(101 + this.type) +'').subscribe(data => {	
