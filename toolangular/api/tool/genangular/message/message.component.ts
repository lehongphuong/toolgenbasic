...
export class MessageComponent implements OnInit {	
  ...
  displayedColumns: string[] = ['id', 'idcandidate', 'idcompany', 'startdate', 'content'];	
   ...
   */	
  getMessage() {	
    this.api.excuteAllByWhat({ 'idCompany': this.api.idCompany }, '50')	
      .subscribe(data => {	
        // set data for table	
...
export class MessageDialog implements OnInit {	
  ...
  onOkClick(): void {	
    ...
    // this.input.born = this.api.formatDate(this.input.born);	
      ...
      this.api.excuteAllByWhat(this.input, ''+ Number(51 + this.type) +'').subscribe(data => {	
   ...
   */	
  onDeleteClick() {	
    this.api.excuteAllByWhat(this.input, '53').subscribe(data => {	
      this.dialogRef.close(true);	
